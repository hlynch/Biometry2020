...
state.fit <- lm(formula = Murder ~ Population, data = state)
ggplot(data = state, aes(x = Population, y = Murder)) + geom_point() +
  geom_abline(aes(intercept = state.fit$coefficients[1], slope = state.fit$coefficients[2])) +
  theme_classic() + theme(text = element_text(size = text.size))
summary(lm(formula = Murder ~ Population, data = state))
```
